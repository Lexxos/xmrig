...
#   ifdef XMRIG_PROXY_PROJECT
    m_results[m_sequence] = SubmitResult(m_sequence * m_runner, result.diff, result.actualDiff(), result.id, 0);
#   else
    m_results[m_sequence] = SubmitResult(m_sequence * m_runner, result.diff, result.actualDiff(), 0, result.backend);
#   endif
